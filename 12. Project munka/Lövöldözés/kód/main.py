...
class Játékos(pygame.sprite.Sprite):
    def __init__(self, csoportok):
        ...
        self.image = pygame.image.load(join('Lövöldözés', 'image', 'player.png')).convert_alpha()
...
async def main():
    ...
    # importálás
    global csillag_kép
    csillag_kép = pygame.image.load(join('Lövöldözés', 'image', 'csillag_kép.png')).convert_alpha()
    global meteor_kép
    meteor_kép = pygame.image.load(join('Lövöldözés', 'image', 'meteor.png')).convert_alpha()
    global lézer_kép
    lézer_kép = pygame.image.load(join('Lövöldözés', 'image', 'lézer.png')).convert_alpha()
    global betű
    betű = pygame.font.Font(join('Lövöldözés', 'image', 'Oxanium-Bold.ttf'), 40)
    global különleges_meteor_kép
    különleges_meteor_kép = pygame.image.load(join('Lövöldözés', 'image', 'különleges_meteor.png')).convert_alpha()
    global robbanás_keretek
    robbanás_keretek = [pygame.image.load(join('Lövöldözés', 'image', 'explosion', f'{i}.png')).convert_alpha() for i in range(21)]
    '''
    lézer_hang = pygame.mixer.Sound(join('Lövöldözés', 'audio', 'lézer.wav'))
    lézer_hang.set_volume(0.02)
    robbanás_hang = pygame.mixer.Sound(join('Lövöldözés', 'audio', 'explosion.wav'))
    robbanás_hang.set_volume(0.02)
    játék_zene = pygame.mixer.Sound(join('Lövöldözés', 'audio', 'game_music.wav'))
    játék_zene.play(loops= -1)
    játék_zene.set_volume(0.04)
    #játék_zene.play(loops= -1)
    '''
    # sprite-ok 
    global összes_sprite 
    összes_sprite= pygame.sprite.Group()
    global meteor_spritek
    meteor_spritek = pygame.sprite.Group()
    global lézer_spritek
    lézer_spritek = pygame.sprite.Group()
    global különleges_meteor_spritek
    ...
    global meteor_esemény
    meteor_esemény = pygame.event.custom_type()
    global különleges_meteor_esemény
